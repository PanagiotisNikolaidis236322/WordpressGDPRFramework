...
<div class="gdpr-dashboard-section">
    ...
    if (isset($consent) && method_exists($consent, 'getConsentStats')):
        $consent_stats = $consent->getConsentStats();
        if (!empty($consent_stats['consent_types']) && is_array($consent_stats['consent_types'])): 
    ?>
        <table class="widefat consent-overview">
            ...
            <tbody>
                <?php foreach ($consent_stats['consent_types'] as $type_key => $type): 
                    if (!is_array($type)) continue;
                ?>
                    <tr>
                        <td><?php echo esc_html($type['label'] ?? __('Unknown', 'wp-gdpr-framework')); ?></td>
                        <td><?php echo esc_html($type['count'] ?? 0); ?></td>
                        <td><?php echo esc_html($type['percentage'] ?? 0); ?>%</td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
